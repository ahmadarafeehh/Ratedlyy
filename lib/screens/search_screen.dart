                          ...
                          itemBuilder: (context, index) {
                            final user =
                                users[index].data() as Map<String, dynamic>? ??
                                    {};
                            final ratings =
                                (user['ratings'] as List<dynamic>?) ?? [];
                            final totalRating = ratings.fold<double>(
                                0.0,
                                (acc, rating) =>
                                    acc +
                                    ((rating as Map)['rating'] as num)
                                        .toDouble());
                            ...
                            child: FutureBuilder<
                              ...
                              builder: (context, usersSnapshot) {
                                ...
                                return FutureBuilder<
                                  ...
                                  future: _filterUsers(
                                      usersSnapshot.data!.docs, blockedUsers),
                                  builder: (context, filteredSnapshot) {
                                    ...
                                    List<Map<String, dynamic>> processedUsers =
                                        [];
                                    for (var userDoc
                                        in filteredSnapshot.data!) {
                                      final userData = userDoc.data()
                                              as Map<String, dynamic>? ??
                                          {};
                                      final ratings = (userData['ratings']
                                              as List<dynamic>?) ??
                                          [];
                                      final totalRating = ratings.fold<double>(
                                          0.0,
                                    ...
                                    processedUsers.sort((a, b) {
                                      int avgCompare = b['averageRating']
                                          .compareTo(a['averageRating']);
                                      if (avgCompare != 0) return avgCompare;
                                      return b['ratingCount']
                                          .compareTo(a['ratingCount']);
                                    });

                                    final topUsers =
                                        processedUsers.take(3).toList();
                    ...
                    Expanded(
                      child: FutureBuilder<QuerySnapshot<Map<String, dynamic>>>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return FutureBuilder<List<QueryDocumentSnapshot>>(
                            future:
                                _filterPosts(snapshot.data!.docs, blockedUsers),
                            builder: (context, filteredSnapshot) {
                              if (filteredSnapshot.connectionState ==
                                  ConnectionState.waiting) {
                                return const Center(
                              ...
                              filteredPosts.sort((a, b) {
                                ...
                                if (result == 0) {
                                  result = ratingsB.length
                                      .compareTo(ratingsA.length);
                                }
                                return result;
                              });
