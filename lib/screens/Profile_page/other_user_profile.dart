...
import 'package:Ratedly/widgets/user_rating_section.dart';
...
import 'package:Ratedly/widgets/user_rating_list_screen.dart';
...
class _OtherUserProfileScreenState extends State<OtherUserProfileScreen> {
  ...
  Future<void> _otherGetData() async {
    ...
    try {
      ...
      final ratings = _otherConvertToList(data['ratings']);
      ...
      // Calculate ratings
      final totalRating = ratings.fold<double>(0.0, (acc, r) {
        final rating = r['rating'];
        if (rating is num) {
          return acc + rating.toDouble();
        } else if (rating is String) {
          return acc + (double.tryParse(rating) ?? 0.0);
        }
        return acc;
      });
      final averageRating =
          ratings.isNotEmpty ? totalRating / ratings.length : 0.0;
      final uniqueVoters = ratings.map((r) => r['raterUserId']).toSet();
      ...
      if (mounted) {
        if (mounted) {
          setState(() {
            ...
            _votersCount = uniqueVoters.length;
            isFollowing =
                followersList.any((f) => f['userId'] == currentUserId);
                ...
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        _buildOtherRatingMetric(),
                        _buildOtherVotersMetric(),
  ...
  Widget _buildOtherRatingMetric() {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => UserRatingListScreen(userId: widget.uid),
          ),
        );
      },
      child: Column(
  ...
  Widget _buildOtherVotersMetric() {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
  ...
  Widget _buildOtherInteractiveMetric(
    ...
    return GestureDetector(
      ...
      child: _buildOtherMetric(validEntries.length, label, Colors.black),
    );
  }

  Widget _buildOtherInteractionButtons() {
    final isPrivateAccount = userData['isPrivate'] ?? false;
    ...
    return Column(
      children: [
        ...
        UserRatingSection(
          ...
          onRatingSubmitted: () => _otherGetData(),
        ),
      ],
    );
  }
  ...
  Widget _buildFollowButton(bool isPrivateAccount) {
    ...
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        backgroundColor: const Color(0xFF333333), // Same color for all states
        foregroundColor: const Color(0xFFd9d9d9), // Same text color
        padding: const EdgeInsets.symmetric(horizontal: 24),
        side: const BorderSide(
          color: Color(0xFF333333), // Match background
        ),
        minimumSize: const Size(100, 40),
      ),
      child: Text(
  ...
  Widget _buildOtherMetric(int value, String label, Color textColor) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Text(
...
// Update the _buildOtherPostsGrid method
  Widget _buildOtherPostsGrid(bool isCurrentUser) {
